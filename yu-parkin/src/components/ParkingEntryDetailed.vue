...
const formattedDateObj = computed(() => {
  return getFormattedDate(new Date(parkingEntry.$createdAt))
});
</script>
  ...
  <Dialog v-model:visible="visible" modal :header="'Carpark: ' + parkingEntry['carpark-name']" class="md:w-96">
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-2">
        ...
        <span class="text-xl font-semibold mt-2">Parked at:</span>
        <span class="text-xl font-normal my-0">{{ formattedDateObj.dayOfWeek }}</span>
        <span class="text-xl font-normal my-0">{{ formattedDateObj.date }}</span>
        <span class="text-xl font-normal my-0">{{ formattedDateObj.time }}</span>
